...
# extract_aSDomains.py - version 1.0
# August 2016
''' 	Parses .gbk output files of antiSMASH 3.0, yielding a txt file of the aSDomains 
		from the output file into an faa-like file. The aSDomain modules are saved in a
		FASTA-like format, where the header reads:
		>NC_011593.1.cluster001_ctg1_orf00220
		for example.
'''
usage = 'extract_aSDomains.py *.gbk'
...
def main():
	if len(sys.argv)<1: #only run if there are actually files that match
		...
		FileList= sys.argv[1:]
		Header = '>' # start each sequence's identifier with >
		#define the start of the sequence by the CDS line
		codingstart = 'aSDomain'
		title_begin = False
		...
		for InfileName in FileList:
			if InfileName.endswith('final.gbk'):
				...
				if "aSDomain_sequences" not in os.listdir("."):
					os.mkdir("aSDomain_sequences")
				HeaderF = Header + InfileName.replace('.gbk','')
				HeaderF = HeaderF.replace('.clu','_clu')
				OutFileName = InfileName + '_aSDomains.mpfa'
				OutFileName = OutFileName.replace('.gbk','')
				OutFile = open(os.path.join('aSDomain_sequences', OutFileName),'w')
				...
				for line in Infile:
					if title_begin: #only do this if 'CDS  ' starts the line
						if line.startswith("                     /locus_tag"):
							...
							OutFile.write(OutfileM + '_aSDomain' + '\n')
