...
def big_cluster_v_cluster(inf3, grab, inkey, cpus, c_list, j):
	...
	with Pool(processes=cpus) as pool:
		results = pool.starmap(parallel_clustermean, zip(c_list, repeat(mx)))
		pool.close()
		pool.join()
		...
		with open(args.output, 'w') if args.output != '-' else sys.stdout as outf:
			...
			for cluster in c_list:
				...
				with open(args.input, 'r') as inf3:
					bigmat = big_cluster_v_cluster(inf3, grab, inkey, cpus, c_list, j)
				# print(bigmat)
