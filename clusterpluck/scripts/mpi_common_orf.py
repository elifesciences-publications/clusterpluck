...
def big_cluster_completeness(inf3, grab, inkey, cluster, cluster_map, tanimoto, c_list, j):
	...
	if __name__ == '__main__':
		if tanimoto:
			results = list(futures.map(partial(parallel_tanimoto, args_list=args_list), c_list))
		else:
			results = list(futures.map(partial(parallel_minicluster, args_list=args_list), c_list))
